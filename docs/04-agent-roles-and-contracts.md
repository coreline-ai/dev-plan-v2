...
식별자를 사용한다. 새 Sol을 만들 수 없으면 `PLAN`, `UPGRADE`,
`EXECUTE`, `RESUME`, `QA`처럼 파일 또는 상태를 바꾸는 모드를 시작하지 않는다.
읽기 전용 `VALIDATE`, `STATUS`는 결정적 스크립트 범위에서 수행할 수 있다.
...
workspace_id: worker-DEV-101-attempt-0001
