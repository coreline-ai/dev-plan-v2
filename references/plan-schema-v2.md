...
[설계 목표](#1-설계-목표) · [파싱 규칙](#2-정본과-파싱-규칙) ·
[메타데이터](#3-계획-메타데이터) · [상태값](#4-상태값) ·
[필수 섹션](#5-필수-상단-섹션) · [Phase 형식](#6-phase-형식) ·
[최종 QA](#7-최종-통합-qa) · [실행 준비](#10-실행-준비-조건) ·
[증빙](#11-evidence-요구-조건) · [기준 상태](#12-git-및-비-git-기준-상태) ·
[이벤트](#13-상태-이벤트와-원자성) · [v1 업그레이드](#14-v1-업그레이드-규칙)
...
| `lead_model`, `qa_model` | 런타임 enum에서 선택한 exact requested model |
...
| `manifest_ignore` | 아래 §12의 고정된 canonical ignore 목록 |
...
| `final_approval_evidence` | 최종 Lead 승인 evidence manifest ref, 승인 전 `NONE` |
| `last_resolution_evidence` | 최근 BLOCK_CLEARED의 조건 해소 evidence, 없으면 `NONE` |
  ...
  - finding_ref: QA-101/A0001/F001
    ...
    opened_by:
      report_manifest:
        ...
        sha256: 0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef
        bytes: 2048
    summary: 재현 가능한 finding 요약
    related_entities: [P1, DEV-101, TEST-101]
    addressed_by: []
    resolved_by: NONE
```
...
`TASK_STARTED`뿐 아니라 `WORKER_REPORTED` 시점에도 저장된 lease가 미래여야 한다.
완료 보고 전에 만료되면 결과를 수락하지 않고 `WORKER_ATTEMPT_INVALIDATED`로
무효화한 뒤 새 attempt를 배정한다.
...
- attempt: 1
  ...
  tested_state_id: sha256:2222222222222222222222222222222222222222222222222222222222222222
  result: PASS
  evidence_manifest:
...
모든 manifest와 `files` 항목은
`dev-plan/evidence/<plan-id>/` 아래에 있어야 하며 path·SHA-256 64자리·byte 크기를
실제 파일과 대조한다. `files`는 비어 있을 수 없다. BASELINE은
`output_state_id`, INPUT은 `input_state_id`, RESULT는 input/output state와
상위 INPUT manifest 참조가 필수다.

엔터티별 필수 role:

| manifest | 필수 role |
|---|---|
| DEV INPUT | `worker_contract`, `workspace_manifest` |
| TEST INPUT | `workspace_manifest` |
| QA INPUT | `qa_contract`, `workspace_manifest` |
| DEV RESULT | `worker_report`, `pre_state`, `post_state`, `diff`, `test_log` |
| TEST RESULT | `test_log`, `post_state` |
| QA RESULT | `qa_response`, `qa_report`, `pre_state`, `post_state` |

`workspace_manifest`, `pre_state`, `post_state` role 파일은
`codex-workspace-manifest/v1`이어야 하며 canonical ignore와 파일 inventory digest가
각 INPUT/RESULT state ID에 일치해야 한다. 승인 시 outer manifest만 다시 해시하지
않고 이 role 파일과 RESULT가 참조한 INPUT manifest까지 재귀 검증한다.
`MANIFEST_GUARDED`의 INPUT/RESULT workspace root는 source root 또는 그 하위일 수
없다. `workspace_root`는 canonical absolute path여야 하며 `workspace_id`는 그
문자열의 SHA-256 앞 24자리와 정확히 같아야 한다.

Worker와 QA 시작 이벤트는 별도의 `codex-runtime-attestation/v1` 참조도 필수다.
attestation에는 `agent_id`, `role`, `requested_model`, 선택적 `worker_tier`,
`supported_models`, `actual_model`, `context_mode: NONE`, canonical
`workspace_root/workspace_id`, model enum snapshot ref, spawn receipt ref와 그
SHA-256, 타임존 포함 `created_at`이 있어야 한다. 별도
`codex-model-enum-snapshot/v1`의 models가 `supported_models`와 같고,
`codex-spawn-receipt/v1`의 agent/model/context/workspace가 attestation과 같아야
한다. 요청 모델은 저장된 runtime enum에 포함되어야 하며 `actual_model`은 요청
모델 또는 런타임이 반환하지 않은 경우의 `NOT_REPORTED`만 허용한다. 이벤트의
agent·model·workspace 값과 attestation이 하나라도 다르면 attempt를 시작하지 않는다.

Phase·Plan·사용자 위험 수용 evidence는 일반 파일 참조가 아니라 plan evidence root
아래의 `codex-approval-evidence/v1` 문서여야 한다.
...
approver_role: USER
decision: ACCEPTED
created_at: 2026-07-25T20:30:00+09:00
statement: 사용자가 명시한 수용 사유
```

`approval_kind`는 `PHASE|PLAN|RISK`다. Phase/Plan은 `approver_role: LEAD`와
`decision: APPROVED`, 위험 수용은 `approver_role: USER`와
`decision: ACCEPTED`를 요구한다. entity ID·plan ID·파일 hash를 이벤트와 대조하며,
최종 승인 시 모든 Phase 승인과 사용자 위험 수용 evidence를 다시 해시·스키마
검증한다.
`PLAN_APPROVED`는 planning/execution BASELINE manifest와 revision binding,
finding을 연 QA report, Worker address report, resolution evidence,
`last_resolution_evidence`까지 다시 검증한다. 완료 상태 전환 후 executable
validator가 생략되더라도 전체 상태-gating evidence graph가 검증되지 않은 채
`COMPLETED`가 될 수 없다.

`PHASE_APPROVED`는 QA input state와 현재 source state가 같은지 확인하고, 같은 state를
가리키는 `codex-workspace-manifest/v1` 및 상태가 `INTEGRATED`인
`codex-integration-journal/v1` 참조를 함께 요구한다. 계획 이벤트 적용이 실패하면
journal로 원복하며, PREPARED crash journal은 현재 트리가 기록된 pre/post 파일
조합일 때만 원복한다. 그 이후 사용자 변경이 관측되면 fail-closed로 중단한다.
journal은 plan 파일·plan ID·Phase ID·승인 직전 plan SHA-256·document version에
더해 해당 Phase DEV들의 canonical `allowed_paths`/`allowed_new_paths` union과
그 digest에 바인딩한다. 통합 도구는 Plan을 직접 파싱해 CLI allowlist가 이 union과
정확히 같은지 잠금 전후에 검증하며, 승인기는 journal의 모든 changed path를 다시
계약과 대조한다. 승인 절차는 `codex-integration-commit/v1` marker를 사용하며,
`COMMITTED` marker가 있는 journal은 재사용하거나 rollback할 수 없다.
상태 갱신기는 source integration lock을 먼저 잡고 그 안에서 plan lock을 잡는다.
두 잠금 사이에는 `dev-plan/evidence/<plan-id>/.control-plane.lock`을 잡아 승인
검증부터 plan 교체까지 manifest·attestation·approval 파일이 바뀌지 않도록 한다.
plan 교체 전 marker를 `COMMITTING`으로 내구성 있게 기록하고 pre/post plan digest를
함께 저장한 뒤, 교체·directory fsync 후 `COMMITTED`로 바꾼다. crash 복구 시
COMMITTING marker와 plan preimage가 일치할 때만 source rollback을 허용하며,
plan postimage가 일치하면 marker를 COMMITTED로 확정하고 rollback을 거부한다.
세 잠금 파일은 unlink 기반 stale 회수 대신 persistent inode의 POSIX advisory
`flock`을 사용한다. owner JSON 기록 중 crash가 나도 커널 잠금이 자동 해제되며,
다음 소유자가 같은 inode를 잠근 뒤 진단 payload를 덮어쓴다.
현재 source와 BASELINE 일치를 승인하는 `PLAN_READY`, `EXECUTION_STARTED`도 같은
source lock 안에서 manifest/revision을 재검증하고 plan 교체를 끝낸다.
...
Phase QA 시작과 승인 시 각 current VALID Worker output이 QA aggregate input으로
상태 그래프상 도달하는지 검증한다. 병렬 merge로 별도 aggregate state가 생기면 각
DEV 소유 경로의 Worker post-state inventory가 aggregate workspace inventory와
일치해야 한다.
통합 도구 자체도 Plan이 `IN_PROGRESS`, 대상 Phase가 `QA`, 모든 DEV가
`WORKER_DONE`, 모든 TEST가 `PASS`, Phase QA current attempt가 `VALID/PASS`인지
검증한다. disposable aggregate state가 그 QA input state와 같지 않으면 source를
수정하지 않는다.

`manifest_ignore`는 planning evidence에 값과 digest를 고정하며 실행 중 변경할 수
없다. v2의 canonical 목록은 다음 다섯 항목과 순서까지 정확히 일치해야 한다.
...
`dev-plan/**`는 append-only evidence와 계획 상태 갱신이 제품 source state ID를
바꾸지 않도록 분리한다. 나머지 캐시·잠금 예외도 위 목록만 허용하며 임의 ignore
추가는 거부한다. attempt evidence append sink는 post-manifest 검증이 끝난 뒤에만
쓰며, 기존 contract·diff·log는 별도 control-plane hash inventory로 보호한다.
`allowed_generated_paths`는 disposable workspace 안에서만 예외이며 실제 작업공간
ignore로 승격되지 않는다.
...
| `WORKER_REPORTED` | DEV `IN_PROGRESS`; lease가 아직 유효; 모든 지정 TASK TEST가 Worker output state에서 PASS; patch/evidence 유효 | DEV `WORKER_DONE`, output state·attempt manifest |
| `WORKER_ATTEMPT_INVALIDATED` | DEV `ASSIGNED|IN_PROGRESS`; timeout·agent 유실·무결성 실패 | attempt `INVALID`, DEV `REWORK` 또는 불명확 시 `BLOCKED` |
| `PHASE_QA_STARTED` | Phase `IN_PROGRESS|QA`, QA `PENDING`; 모든 DEV `WORKER_DONE|DONE`, TEST `PASS`; Worker output→aggregate provenance와 Sol/격리 계약 유효 | Phase `QA`, QA `RUNNING`, current_attempt+1 |
...
| `FINDING_RESOLVED` | finding `OPEN`; 후속 QA current attempt `VALID/PASS`; resolution evidence 유효 | finding `RESOLVED`, QA attempt와 evidence 연결 |
...
| `TASK_ASSIGNED` | `task_id`, tier/exact model, agent ID, input state, contract manifest, lease, source 밖 canonical `workspace_root`, 파생 `workspace_id`, runtime attestation |
| `TASK_STARTED` | `task_id`, 현재 attempt·agent ID·input state·lease 만료 시각, started_at |
...
| `PHASE_QA_STARTED` | `phase_id`, `qa_id`, agent/context/model/deadline, INPUT manifest, source 밖 canonical `workspace_root`, 파생 `workspace_id`, runtime attestation |
...
| `PHASE_APPROVED` | `phase_id`, QA input state, `approval_evidence`, integrated workspace manifest, rollback journal |
| `PLAN_QA_STARTED` | `qa_id=QA-FINAL`, final input state, agent/context/model/deadline, INPUT manifest, source 밖 canonical `workspace_root`, 파생 `workspace_id`, runtime attestation |
...
| `FINDING_RESOLVED` | qualified finding ref, PASS `qa_id`, QA input state, resolution evidence |
...
| `PLAN_APPROVED` | final QA input state, `approval_evidence`, residual risks, user risk approvals |
