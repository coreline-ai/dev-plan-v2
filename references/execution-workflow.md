...
[기본 흐름](#1-기본-흐름) · [사전 검사](#2-공통-사전-검사) ·
[PLAN](#3-plan) · [UPGRADE](#4-upgrade) · [VALIDATE](#5-validate) ·
[EXECUTE](#6-execute) · [Independent QA](#7-independent-qa) ·
[재작업](#8-재작업) · [완료](#9-phase와-최종-완료) ·
[RESUME](#10-resume) · [QA·STATUS](#11-qa와-status-전용-모드) ·
[중단 조건](#12-실행-중단-조건)
...
4. 모드와 허용 부작용을 확정한다.
5. `PLAN`, `UPGRADE`, `VALIDATE`, `STATUS`는 현재 실행자가 결정적 스크립트로
   처리하며 Worker/QA/대체 Lead를 생성하지 않는다.
6. `EXECUTE`, `RESUME`, `QA`에서 현재 실행자를 Sol로 확인하지 못하면
   `fork_turns: "none"`과 실제 Sol 모델 식별자로 새 Lead를 생성한다. 생성할 수
   없으면 임의 대체하지 않고 `BLOCKED`로 보고한다.
...
Lead는 전체 계획의 자유 해석을 맡기지 않고 `references/agent-contracts.md`의
...
Lead는 `WORKER_REPORTED`를 받을 때도 lease를 다시 검사한다. 시작 때 유효했더라도
보고 전에 만료됐으면 완료로 수락하지 않는다.

### 6.5 결정적 작업공간 통합과 복구

`MANIFEST_GUARDED`에서는 임의 복사·patch 명령 대신 패키지의
`workspace_guard.py`를 사용한다.

1. `snapshot`으로 source baseline을 생성한다.
2. `prepare-copy`로 source 밖 disposable workspace와 같은 state의 baseline을 만든다.
   canonical root가 source와 같거나 그 하위이면 Worker·Phase QA·최종 QA 시작과
   evidence 재검증을 모두 거부한다. workspace ID도 canonical root에서 재계산한다.
3. Worker 종료 후 `verify`로 allowed/allowed-new 경로만 달라졌는지 검사한다.
4. `integrate`에 source/workspace baseline, 허용 경로, source의
   `dev-plan/evidence/` 아래 output manifest와 rollback dir, plan 파일·ID·Phase
   ID·현재 plan SHA-256·document version을 전달한다.
5. 도구는 Plan을 파싱해 Phase/DEV/TEST/QA가 post-QA integration 상태인지,
   disposable state가 PASS QA input과 같은지, 전달된 allowlist가 Phase DEV 경로
   계약 union과 정확히 같은지 확인한다. 이어 source→evidence→plan lock 안에서
   Plan SHA/version/contract/QA state와 source preimage CAS를 다시 확인한 뒤
   `PREPARED` journal과 백업을 내구성 있게 저장한다.
6. 통합 state가 QA input state와 같을 때만 `PHASE_APPROVED`를 적용한다.
7. 계획 이벤트가 실패하면 즉시 `rollback --journal .../journal.json`을 실행한다.
8. 승인 절차에서 상태 갱신기가 같은 디렉터리에 `COMMITTED.json`을 생성한다.
   `status: COMMITTED` marker가 있는 journal은 과거 이벤트 replay나 수동
   rollback에 사용할 수 없다.

Phase 승인에는 source integration lock → plan evidence control-plane lock → plan
lock 순서의 중첩 잠금을 사용한다. evidence를 생성·통합·rollback하는 결정적 도구도
같은 control-plane lock 규약을 따른다.
잠금은 persistent inode에 대한 POSIX `flock`이므로 owner payload가 비었거나
부분 기록된 채 process가 종료되어도 커널이 잠금을 해제한다. unlink 기반 stale
회수는 사용하지 않는다.
plan 교체 전에 marker를 `COMMITTING`으로 기록하고 pre/post plan digest를 저장한다.
교체가 끝나면 같은 잠금 구간에서 `COMMITTED`로 확정한다. crash 시 plan이 preimage면
marker를 제거하고 source를 복원할 수 있지만, postimage면 rollback을 금지하고
commit을 확정한다.

PREPARED crash 복구는 현재 source가 기록된 pre/post 파일들의 조합일 때만 가능하다.
사용자나 다른 프로세스가 통합 대상 또는 보호 경로를 바꿨다면 자동 복원하지 않고
`BLOCKED`로 보고한다. 성공한 계획 상태 이벤트의 integration journal은 Phase 승인
증빙으로 보존한다.

Phase QA input은 각 current VALID Worker output에서 상태 그래프로 도달해야 한다.
병렬 merge가 별도 aggregate state를 만들면 Worker 결과 manifest의 소유 경로
post-state 항목이 QA INPUT의 aggregate workspace manifest와 정확히 같아야 한다.
